-- Redefine check_in_user to include richmond@guava.earth as admin
...
begin
  ...
  if caller_email ilike 'admin@%' 
     ...
     or caller_email = 'richmond@guava.earth'
...
-- Update database profile role for richmond@guava.earth if they already exist
update public.profiles
set socials = coalesce(socials, '{"instagram": "", "twitter": "", "discord": ""}'::jsonb) || '{"role": "admin"}'::jsonb
where email = 'richmond@guava.earth';
